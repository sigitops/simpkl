...
// Jawaban server sudah memuat baris lengkap hasil penyimpanan, termasuk ID
// baru dan nilai yang dinormalkan. Menyuntikkannya ke singgahan membuat tabel
// langsung benar tanpa perjalanan server kedua; penyegaran senyap tetap
// berjalan sesudahnya sebagai pemeriksa.
batalkanPaketData();
suntikBaris('getMasterData', [AppState.sessionToken, entitas], res.data);
if (entitas === 'Siswa' || entitas === 'GuruPembimbing') {
lupakanKunci('getMasterData', [AppState.sessionToken, 'Akun']);  // akun otomatis ikut terbentuk
}
muatTabelMaster();
} catch (err) { sembunyikanSibuk(); toast(err.message, 'error'); }
}
...
if (res.success) {
batalkanPaketData();
suntikBaris('getMasterData', [AppState.sessionToken, entitas], null, [id]);
muatTabelMaster();
}
} catch (err) { sembunyikanSibuk(); toast(err.message, 'error'); }
}
...
batalkanPaketData();
// Sebagian id bisa saja dilewati server karena masih terpakai. Penyegaran
// senyap yang menyusul akan mengembalikan baris yang ternyata tidak jadi
// terhapus, jadi tabel tetap jujur.
suntikBaris('getMasterData', [AppState.sessionToken, entitas], null, ids);
muatTabelMaster();
} catch (err) { sembunyikanSibuk(); toast(err.message, 'error'); }
}
