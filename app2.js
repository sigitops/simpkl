...
let POPUP_KAMERA = null;
function terimaFotoPopup(e) {
if (!e.data || e.data.tipe !== 'FOTO_PRESENSI') return;
if (POPUP_KAMERA && !POPUP_KAMERA.closed) { try { POPUP_KAMERA.close(); } catch (x) {} }
POPUP_KAMERA = null;
window.removeEventListener('message', terimaFotoPopup);
AppState.fotoTerambil = e.data.foto;
tampilkanPratinjauFoto(e.data.foto);
toast('Foto berhasil diambil dari kamera.', 'success');
}
function tutupPopupKamera() {
try { if (POPUP_KAMERA && !POPUP_KAMERA.closed) POPUP_KAMERA.close(); } catch (e) {}
POPUP_KAMERA = null;
window.removeEventListener('message', terimaFotoPopup);
}
function bukaPopupKamera() {
const l = 520, t = 620;
const kiri = Math.round((window.screenX || 0) + ((window.outerWidth || 1280) - l) / 2);
const atas = Math.round((window.screenY || 0) + ((window.outerHeight || 800) - t) / 2);
POPUP_KAMERA = window.open('', 'KameraPresensi',
'width=' + l + ',height=' + t + ',left=' + kiri + ',top=' + atas + ',resizable=yes,scrollbars=no');
if (!POPUP_KAMERA) {
bukaModal('Jendela Kamera Diblokir', `
<div class="alert alert-warning" style="margin-bottom:14px">
<span class="mi">block</span>
<div><strong>Browser memblokir jendela pop-up</strong>
<p style="margin-top:6px">Kamera dibuka di jendela terpisah agar tidak terhalang
pembatasan iframe. Izinkan pop-up untuk situs ini lalu coba lagi.</p></div>
</div>
<div class="info-eyebrow" style="margin-bottom:8px">Cara mengizinkan</div>
<ol style="margin:0;padding-left:20px;font-size:14px;line-height:1.7;color:var(--on-surface-variant)">
<li>Klik ikon pop-up terblokir di ujung kanan bilah alamat</li>
<li>Pilih <b>Selalu izinkan pop-up dari situs ini</b></li>
<li>Klik <b>Aktifkan Kamera</b> sekali lagi</li>
</ol>`,
[{ label: 'Tutup', kelas: 'btn-outline', aksi: tutupModal },
{ label: '<span class="mi">refresh</span> Coba Lagi', kelas: 'btn-primary',
aksi: () => { tutupModal(); bukaPopupKamera(); } }]);
return;
}
window.removeEventListener('message', terimaFotoPopup);
window.addEventListener('message', terimaFotoPopup);
POPUP_KAMERA.document.write(
'<html><body style="margin:0;background:#0F1416;color:#E1E3E4;font-family:sans-serif;' +
'display:grid;place-items:center;height:100vh"><p>Menyiapkan kamera…</p></body></html>');
const tulis = function (html) {
if (!POPUP_KAMERA || POPUP_KAMERA.closed) return;
POPUP_KAMERA.document.open();
POPUP_KAMERA.document.write(html);
POPUP_KAMERA.document.close();
};
if (AppState.htmlKamera) { tulis(AppState.htmlKamera); return; }
panggil('htmlKameraPopup', AppState.sessionToken)
.then(function (res) {
if (!res || !res.success) { tutupPopupKamera(); toast('Gagal menyiapkan kamera.', 'error'); return; }
AppState.htmlKamera = res.data;
tulis(res.data);
})
.catch(function (e) { tutupPopupKamera(); toast(e.message, 'error'); });
}
...
$('btnKamera').hidden = true;
$('btnUlangFoto').hidden = false;
evaluasiTombolPresensi();
}
function ulangiFoto() {
AppState.fotoTerambil = null;
...
$('btnUlangFoto').hidden = true;
const btn = $('btnKamera');
btn.hidden = false;
btn.innerHTML = '<span class="mi">photo_camera</span> Aktifkan Kamera';
btn.onclick = aktifkanKamera;
evaluasiTombolPresensi();
}
